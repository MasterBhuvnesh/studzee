...
**Live on real infrastructure since 29-08-2026.** The migration is applied to
Neon, the knowledge base is indexed at 25 passages, and the support agent has
been driven against it. What is in the vector store is listed in
`BACKEND/src/services/ai/kb/KB-CONTENTS.md`, regenerated by every reindex.

**The test suite must never be pointed at production.** `.env` now holds real
Neon, Atlas and Upstash credentials. `globalSetup.ts` resolves
`MONGO_URI_TEST || MONGO_URI || localhost`, and today it lands on localhost
only because `.env` is not loaded before it runs. Exporting `MONGO_URI` or
`REDIS_URL` into a shell before `npm test` would point the integration suite,
which flushes Redis and writes collections, at production. Verified on
29-08-2026 that a plain `npm test` resolves all three to localhost. Do not
change that ordering without rechecking it.
...
list below. That endpoint is not harmless to poke at: approving a `document`
draft publishes it **and pushes a notification to every registered device**, by
owner instruction on 29-08-2026. It is the one place outreach follows from an
action rather than from its own draft, and it is still gated on a deliberate
approval.
