...
- **Two Clerk SDKs are in use.** `@clerk/express` for the middleware and `@clerk/clerk-sdk-node` for `clerkClient`, and the latter is end of life. Consolidating onto `@clerk/backend` is outstanding.
