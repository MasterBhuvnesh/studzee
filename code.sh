...
# Add your other service names to this regex pattern, e.g., ^(notification|backend|mobile|website)$
if [[ ! "$SERVICE_NAME" =~ ^(notification|backend|mobile|website)$ ]]; then
    echo -e "${RED}Error: Invalid service name '${SERVICE_NAME}'. Please use 'notification', 'backend', 'mobile', or 'website'.${NC}"
    exit 1
fi
